...
def testing():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ssd_model = ssd_resnet.SSD_resnet(sess,False)
...
def training():
    ...
    train_times = int(cfg.train_epoch*(data_num/cfg.train_batch))
    print("=================  start training  ================")
    print("=================  training batch: %d  =================" %(batch_size))
    print("=================  training epoch: %d  =================" %(cfg.train_epoch))
    print("=================  training total times: %d  =================" %(train_times))
    running_count = 0
    ...
    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.9)
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ssd_model = ssd_resnet.SSD_resnet(sess,True)
        sess.run(tf.global_variables_initializer())
        saver = tf.train.Saver(var_list=tf.trainable_variables(), max_to_keep=20)
        if os.path.exists('./session_params/session.ckpt.index') :
            print('\nStart Restore')
            saver.restore(sess, './session_params/session.ckpt')
            print('\nEnd Restore')
        ...
        while running_count < train_times:#((min_loss_location + min_loss_class) > 0.001 and running_count < 100000):
            running_count += 1
            print("=================  training times: %d  =================" %(running_count))
            ...
            if len(train_data) > 0:
                ...
                if running_count % 100 == 0:
                    saver.save(sess, './session_params/session'+datetime.datetime.now().strftime('%m_%d_%H_%M')+'.ckpt')
